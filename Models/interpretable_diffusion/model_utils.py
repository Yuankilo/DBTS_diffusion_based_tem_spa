...
def extract2(a, t, x_shape):
    # 获取 batch size
    b, *_ = t.shape
    # 执行 gather 操作，确保 index 张量扩展正确
    out = a.gather(dim=-1, index=t.unsqueeze(-1).expand(b, *x_shape[1:]))
    # 调整形状
    out = out.reshape(b, *x_shape[1:])
    return out
...
class LearnablePositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        #print(f"Position encoding size: {self.pe.size()}")
        seq_len = x.size(1)
        #print(seq_len)
        if x.size(1) > self.pe.size(1):
            raise RuntimeError(f"Sequence length {x.size(1)} exceeds maximum length {self.pe.size(1)}")
        # print(x.shape)
        x = x + self.pe[:, :seq_len, :]
